...
public class WandOfMending implements Listener {

    /*************************************
     * Rewritten on 16/08/2024 at 18h00. *
     *************************************/

    private final Map<UUID, Integer> cooldowns = new HashMap<>(); // Map pour stocké les cooldown des joueurs.
    ...
    private final double HEALTH_REGEN_AMOUNT = 7.5; // Nombre de vie régenéré à chaque DELAY_TIME.
    private final int DELAY_TIME = 1; // Nombre de temps avant chaque répétition de la tache en secondes.
    private final int EXECUTE_TIME = 5; // Nombre de seconde avant la fin de la tâche.

    @EventHandler
    public void WandOfMending_Use(PlayerInteractEvent e) {
        ...
        if (!name.equals(color.set("&bSceptre de Mending"))) {
            return;
        }
